...
config_env_1.subtask(task_names_1.TASK_COMPILE_SOLIDITY_GET_SOLC_BUILD)
    ...
    let { compilerPath, platform, } = await downloader.getDownloadedCompilerPath(solcVersion);
    // when using a native binary, check that it works correctly
    // it it doesn't, force the downloader to use solcjs
    if (platform !== downloader_1.CompilerPlatform.WASM) {
        ...
        if (!solcBinaryWorks) {
            ...
            const { compilerPath: solcJsCompilerPath, } = await solcJsDownloader.getDownloadedCompilerPath(solcVersion);
...
config_env_1.subtask(task_names_1.TASK_COMPILE_SOLIDITY_LOG_COMPILATION_ERRORS)
    ...
    if (hasConsoleErrors) {
        console.error(chalk_1.default.red(`The console.log call you made isn’t supported. See https://usehardhat.com/console-log for the list of supported methods.`));
        console.log();
    }
});
/**
