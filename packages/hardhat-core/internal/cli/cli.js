...
async function main() {
    ...
    try {
        ...
        if (showStackTraces) {
            ...
            if (!isHardhatError) {
                console.error(`If you think this is a bug in Hardhat, please report it here: https://usehardhat.com/reportbug`);
            }
            if (errors_1.HardhatError.isHardhatError(error)) {
                const link = `https://usehardhat.com/${errors_list_1.getErrorCode(error.errorDescriptor)}`;
